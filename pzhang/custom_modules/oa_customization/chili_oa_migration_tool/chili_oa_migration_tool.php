...
function com_load_chiliproject_issue_data_form(&$form_state = NULL){
  ...
  if ($form_state['clicked_button']['#id'] == 'edit-action-submit') {
  	if ($mapping){
	    ...
	    com_mapping_chili_project_id_to_task_sections('Task Section');
    ...
    $sql = "select id from " . $chili_db . ".issues order by id";
		...
		if(!$round2){
			while ($rc = $result ->fetchObject()){
				$id = $rc -> id;
		    $operations[] = array('com_load_chiliproject_issue_data_batch', array($id, $test, 1,$temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping));
			}
		}
    $result = db_query($sql);
    while ($rc = $result ->fetchObject()){
			$id = $rc -> id;
			$round = 2;
	    $operations[] = array('com_load_chiliproject_issue_data_batch', array($id, $test, $round,$temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping));
      //$operations[] = array('com_test', array($id, $test, $round,$temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping));
		}
	  $batch = array(
      ...
      'file' => 'chili_oa_migration_tool.php',
	  );
    drupal_set_message(_dump($operations, 'purple', TRUE));
	  batch_set($batch);
	  batch_process('chili_oa_migration_tool.php');
    //drupal_set_message(_dump($operations, 'purple', TRUE));
...
function com_test($id, $testing,$round, $temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping, $content){
  
	drupal_set_message(_dump("$chili_db, $offcial_attacment_file_folder", 'purple', TRUE));
}

function com_load_chiliproject_issue_data_batch($id, $testing,$round, $temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping, $content){
  ...
  if (!isset($context['sandbox']['progress'])) {
	...
	if ($round == 2) {
		com_load_chiliproject_issue_data_2($id, $testing,$temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping);
	}
	else{
    com_load_chiliproject_issue_data($id, $testing, $temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping);
	}
...
function com_load_chiliproject_issue_data($id, $testing, $temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping){
...
function com_load_chiliproject_issue_data_2($id, $testing, $temp_attachment_file_folder,$offcial_attacment_file_folder,$chili_db,$custom_fields_mapping){
...
function com_load_chiliproject_wiki_data_form(&$form_state = NULL){
 ...
 $test = isset($form_state['values']['test']) ? $form_state['values']['test'] : 0;
  ...
  if ($form_state['clicked_button']['#id'] == 'edit-action-submit') {
  	if ($mapping){
			drupal_set_message("Stating mapping chili porject id to the Task sections...");
	    com_mapping_chili_project_id_to_task_sections('Wiki Section');
	    drupal_set_message("Mapping chili porject id to the Task sections is done.");
		}
